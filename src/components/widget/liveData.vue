<template>
  <div id="liveData">
    <div
      v-if="!loadedData"
    >
      <q-skeleton type="rect" square height="6.2em" class="q-mx-md border-gray"/>
      <div class="row justify-center q-mb-lg q-pt-xs q-mt-lg">
        <q-skeleton type="rect" width="16ch" animation="pulse-x" height="1.5em"/>
      </div>
      <q-skeleton type="rect" square height="13.8em" class="q-mx-md border-gray"/>
      <div class="row justify-center q-mb-lg q-pt-xs q-mt-lg">
        <q-skeleton type="rect" width="16ch" animation="pulse-x" height="1.5em"/>
      </div>
      <q-skeleton type="rect" square height="17.5em" class="q-mx-md border-gray"/>
      <div class="row justify-center q-mb-lg q-pt-xs q-mt-lg">
        <q-skeleton type="rect" width="16ch" animation="pulse-x" height="1.5em"/>
      </div>
      <div class="row q-col-gutter-y-sm q-mx-md">
    ...
    >
      ...
      <div v-if="Array.isArray(info.events) && info.events.length" class="q-px-md">
        <template
          v-for="(event, index) in info.events"
        >
          <div
          ...
          >
            <div class="col-xs-12 q-pa-md relative-position border-gray bg-light-bright-overlay">
      ...
      <div class="text-h6 michroma text-center q-mb-md q-mt-lg" v-if="typeof info.sortie === 'object'">
        Sortie
      </div>
      <div v-if="typeof info.sortie === 'object'" class="relative-position q-mx-md">
        <div class="border-gray bg-light-bright-overlay q-pa-md">
          <span
            v-if="typeof info.sortie.faction !== 'undefined' && info.sortie.faction.length" color="orange-8"
            :label="info.sortie.faction"
            class="q-pr-md q-pb-xs q-ml-md block-underline inline-block"
          >
            <span v-if="typeof info.sortie.boss !== 'undefined' && info.sortie.boss.length" class="text-subtitle-1 q-mr-xs" >{{ info.sortie.boss }}</span> - <q-badge class="q-ml-xs" :label="info.sortie.faction"/>
          </span>
          <q-carousel
          ...
          >
            <q-carousel-slide
              v-for="(step, index) in info.sortie.variants"
              :name="index"
              :key="'sortie_'+index"
              class=""
            >
      ...
      <div class="text-h6 michroma text-center q-mb-md q-mt-lg" v-if="Array.isArray(info.fissures)">
        Fissures
      </div>
      <div
        v-if="Array.isArray(info.fissures) && info.fissures.length && info.fissures.filter((fissure) => fissure.active).length"
        class="q-px-md"
      >
        <q-list
          bordered
        ...
        >
          <q-expansion-item
            v-for="(fissure, index) in formatFissures(info.fissures)"
            :key="index"
            group="relics"
          >
      ...
      <div class="text-h6 michroma text-center q-mb-md q-mt-lg" v-if="Array.isArray(info.invasions) && info.invasions.length">
        Invasions
      </div>
      <div class="q-px-md row q-col-gutter-y-sm" v-if="Array.isArray(info.invasions) && info.invasions.filter(function (invasion) {return !invasion.completed && invasion.eta !== 'Infinityd'}).length">
        <template
          v-for="(factionInvasion, invasionName) in formatInvasions(info.invasions)"
        >
        <div
        ...
        >
          <q-expansion-item
            class="bg-light-bright-overlay border-gray tgi-invasion-item"
          ...
          >
            <template
              v-slot:header
            >
              <div class="row full-width q-px-md q-pt-md q-pb-xs">
                <div class="col-xs-12">
            ...
            <div class="invasion-infos-body t-normal-ease">
              <div class="row q-px-md q-pb-md q-col-gutter-x-lg">
                <div
